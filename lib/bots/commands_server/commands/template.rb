...
module CommandsServer
  class Template < TemplateCommand
    ...
    def run(args = nil)
      ...
      validate_template_sub_command param if sub_command == 'validate'

      create_template_sub_command param if sub_command == 'create'
    end
    ...
    def build_nutella_json(name, version, type, description, repo)
      ...
      json['description'] = description unless description.empty?
      json['repo'] = repo unless repo.empty?
      json
    end
  end
end
